...
__all__ = [
    ...
    'is_admin_windows', 'request_elevation_windows', 'is_termux', 'get_platform_name'
]
...
def get_platform_name() -> str:
    ...
    Returns:
        Platform name (e.g., 'Windows', 'Linux (Termux)', 'macOS').
    """
    if sys.platform == "win32":
        return "Windows"
...
def get_os_package_manager() -> str:
    ...
    Returns:
        Package manager command (e.g., 'apt install', 'winget install', 'pkg install').
    """
    # Check for Termux (Android) first
    if is_termux():
        return "pkg install"
...
def is_termux() -> bool:
    ...
    if os.environ.get("PREFIX", "").endswith("usr"):
        return True
